...
def mutate_3opt(individual, mutation_rate):
    """3-Opt Mutation"""
    if random.random() < mutation_rate:
        size = len(individual)
        i, j, k = random.sample(range(size), 3)
        i, j, k = sorted([i, j, k])
        segment1 = individual[:i]
        segment2 = individual[i:j]
        segment3 = individual[j:k]
        segment4 = individual[k:]
        individual = segment1 + segment3 + segment2 + segment4
    return individual
...
def mutate(individual, mutation_rate):
    ...
    individual = mutate_3opt(individual, mutation_rate)
    return individual
