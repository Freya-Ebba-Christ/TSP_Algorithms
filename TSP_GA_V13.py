...
def mutate_operator_selection(individual, mutation_rate):
    """Mutation Operator Selection"""
    operators = [mutate_swap, mutate_insert, mutate_reverse_sequence]
    selected_operator = random.choices(operators, weights=[0.4, 0.3, 0.3])[0]
    return selected_operator(individual, mutation_rate)

def opt_3_exchange(route, distance_matrix):
    """3-Opt Exchange for local optimization"""
...
def tabu_search(route, distance_matrix, tabu_list_size=10, max_iterations=100):
    ...
    while iterations_without_improvement < max_iterations:
        ...
        for neighbor in neighbors:
            neighbor_distance = calculate_route_length(neighbor, distance_matrix)
...
def generate_neighbors(route):
    """Generate neighboring solutions by applying 2-opt exchange."""
    neighbors = []
...
def mutate(individual, mutation_rate):
    """Combining multiple mutation operators"""
    individual = mutate_operator_selection(individual, mutation_rate)
    return individual
...
def breed_population(matingpool, elite_size):
    ...
    for i in range(elite_size, len(matingpool)):
        if random.random() < 0.5:  # 50% chance of using ordered crossover
...
def dynamic_crossover_rate(current_gen, total_gens, initial_rate, final_rate):
    """Adjusts the crossover rate dynamically over generations."""
    return initial_rate + (final_rate - initial_rate) * current_gen / total_gens

def genetic_algorithm(num_cities, pop_size, elite_size, mutation_rate, generations, initial_crossover_rate, final_crossover_rate):
    ...
    for i in range(generations):
        crossover_rate = dynamic_crossover_rate(i, generations, initial_crossover_rate, final_crossover_rate)
        pop = next_generation(pop, elite_size, mutation_rate, distance_matrix)
        for j in range(len(pop)):
            pop[j] = opt_3_exchange(pop[j], distance_matrix)
            pop[j] = tabu_search(pop[j], distance_matrix)  # Apply Tabu Search after 3-Opt Exchange
            if random.random() < crossover_rate:
                pop[j] = cx_ordered_crossover(pop[j], random.choice(pop))
...
best_route = genetic_algorithm(num_cities, population_size, elite_size, mutation_rate, generations, initial_crossover_rate, final_crossover_rate)
