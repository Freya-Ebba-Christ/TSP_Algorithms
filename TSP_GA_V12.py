...
def mutate(individual, mutation_rate):
    ...
    individual = mutate_swap(individual, mutation_rate)
    individual = mutate_insert(individual, mutation_rate)
    individual = mutate_reverse_sequence(individual, mutation_rate)
    return individual
...
def breed_population(matingpool, elite_size, crossover_rate):
    ...
    for i in range(elite_size, len(matingpool)):
        if random.random() < crossover_rate:  # Use dynamic crossover rate
            ...
            child = pool[i]  # Use parent without crossover
        children.append(child)
    return children

def dynamic_crossover_rate(current_gen, max_gen, initial_rate, final_rate):
    """Calculates dynamic crossover rate based on the current generation."""
    return initial_rate + (final_rate - initial_rate) * (current_gen / max_gen)
