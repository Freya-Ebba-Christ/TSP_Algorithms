...
def breed(parent1, parent2):
    child = []
    # Select a random crossover operator
    crossover_operator = random.choice(['PMX', 'OX', 'CX'])
...
def pmx_crossover(parent1, parent2):
    ...
    if a > b:
        a, b = b, a
    mapping = dict(zip(parent1[a:b], parent2[a:b]))
    child = [mapping.get(gene, gene) for gene in parent1]
    for i in range(a, b):
        child[i] = parent2[i]
    return child
